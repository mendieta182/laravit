<template>
    <admin-layout>        
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="font-bold text-md text-black dark:text-white ml-2">
                                            {{ $page.props.language['Roles'] }}
                                        </span>
                                        <span class="text-sm text-gray-500 dark:text-white ml-2">
                                            Showing {{$page.props.roles.from}} to {{$page.props.roles.to}} of {{$page.props.roles.total}} results
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex flex-1">
                                    ...
                                    <div class="flex space-x-3 ml-3">
                                        <div class="relative">
                                            <button type="button" class="inline-flex items-center transition-default whitespace-nowrap text-sm  pl-4 pr-3 py-2 border leading-5 font-medium shadow-sm rounded-md border-gray-300 text-gray-700 bg-white hover:bg-gray-50" aria-expanded="false">
                                                <div class="flex items-center">
                                                    <span class="inline-block rounded-full w-2 h-2 mr-2.5 bg-green-400"></span>
                                                    Status
                                                </div> 
                                    ...
                                    <div class="w-full overflow-x-auto">
                                        <table class="w-full whitespace-no-wrap table-auto">
                                            <thead>
                                                ...
                                                <th class="max-w-md mx-auto">Guard name</th>
                                                <th class="max-w-md mx-auto">created_at</th>
                                                <th class="px-4 py-3">{{ $page.props.language['Permissions'] }}</th>
                                            ...
                                            >
                                                <tr v-for="(role,index) in roles.data" :key="index" class="text-gray-700 dark:text-gray-400">
                                                    <td class="px-4 py-3 text-sm">
                                                        {{ (index+1) + $page.props.roles.current_page * $page.props.roles.per_page - $page.props.roles.per_page }}
                                                    </td>
                                                    <td class="px-4 py-3">
                                                    ...
                                                    <td class="px-4 py-3 text-xs justify-between w-1/3">
                                        ...
                                        <div class="flex justify-center px-4 py-3 bg-gray-50 space-x-1 text-coolGray-800">
                                            ...
                                            <Link v-if="$page.props.roles.current_page>1" :href="$page.props.roles.first_page_url">
                                            ...
                                            <Link v-if="$page.props.roles.current_page>1" :href="$page.props.roles.prev_page_url">
                                            ...
                                            <Link v-for="(link, k) in roles.links.slice(this.offset,$page.props.roles.last_page+1)" :key="k" :href="link.url">
                                            ...
                                            <Link v-if="$page.props.roles.current_page < $page.props.roles.last_page" :href="$page.props.roles.next_page_url">
                                            ...
                                            <Link v-if="$page.props.roles.current_page < $page.props.roles.last_page" :href="$page.props.roles.last_page_url">
                                        ...
                                        <!-- <div class="card-footer clearfix">
                                            <pagination :links="roles.links" :total="roles.total" :to="roles.to" :from="roles.from"></pagination>
                                        </div> -->
                                    </div>
                                </div>
                            </div>
        ...
        <jet-dialog-modal v-if="estadoModalCreate == true" :show="estadoModalCreate" @close="estadoModalCreate = false" max-width="lg">
            <template #title>
                <p class="text-lg flex items-center justify-center font-bold uppercase">Create new role</p>
            </template>
            <template #content>
                <form class="px-2 bg-white dark:bg-gray-700 rounded" @submit.prevent="createRole">
                    <div class="mb-4">
                            ...
                            <input
                                v-model="form.name"
                                :class="form.errors.name ? 'border-red-500 dark:border-red-400':''"
                                ...
                                placeholder="Name"
                            />
                            <p v-if="form.errors.name" class="text-xs italic text-red-500 dark:text-red-400">{{ form.errors.name }}</p>
                    </div>
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-3 gap-2 bg-gray-100 dark:bg-gray-800 rounded">
                            <div v-for="permission in permissions" :key="permission.id">
                                <label class="inline-flex items-center text-xs m-2">
                                    <input type="checkbox" v-model="form.permissions" class="form-checkbox rounded" :value="permission.id">
                    ...
                    <div class='flex items-center justify-center'>
                        <button class="flex bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full" @click="closeModal">
                            <span class="text-sm">Cancel</span>
                        </button>
                        <button type="submit"
                        :disabled="!form.name||form.processing" :class="!form.name ? 'disabled:opacity-50 disabled cursor-not-allowed':''" 
        ...
        <jet-dialog-modal v-if="estadoModalShow == true" :show="estadoModalShow" @close="estadoModalShow = false" max-width="lg">
            ...
            <template #content>
                    ...
                    <div class="mb-4">
                        <label class="block mb-2 text-sm font-bold text-gray-700 dark:text-white" for="permissions">
                            {{ $page.props.language['Permissions'] }}
                        </label>  
                        <div class="grid grid-cols-3 gap-2 bg-gray-100 dark:bg-gray-800 rounded">
                            <div v-for="permission in permissions" :key="permission.id">                       
                                <label class="inline-flex items-center text-xs m-2">
                                    ...
                                    disabled/>
                                    <span class="mx-2" :for="permission.id">{{ permission.name }}</span>
                                </label>
                            </div>
                        </div>                         
                    </div>
                ...
                <form class="px-2 bg-white dark:bg-gray-700 rounded" @submit.prevent="updateRole">
                    <div class="mb-4">
                            ...
                            <input
                                ...
                                placeholder="Name"
                            />
                            <p v-if="form.errors.name" class="text-xs italic text-red-500 dark:border-red-400">{{ form.errors.name }}</p>
                    </div>
                    <div class="mb-4">
                        <label class="block mb-2 text-sm font-bold text-gray-700 dark:text-white" for="permissions">
                            {{ $page.props.language['Permissions'] }}
...
<script>
    ...
    export default {
        ...
        props: ['roles','permissions','nameSearch','perPage'],
        components: {
            AdminLayout,
            Icon,
            Link,
        ...
        methods: { 
            ...
            showModal(role){
                ...
                this.selectedPermissions=role.permissions.map(item => item.id)
            },
            editModal(role){
                this.form.clearErrors()
                this.estadoModalCreate = false
            ...
            updateRole(){
                this.form.patch(this.route('admin.roles.update',this.form.id, this.form.permissions=this.selectedPermissions),{
                    preserveScroll:true,
                    onSuccess:()=>{
                        this.form.reset()
                        this.closeModal() 
